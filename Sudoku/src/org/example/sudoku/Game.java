...
public class Game extends Activity{
	...
	private int[] getPuzzle(int diff){
		...
		case DIFFICULTY_CONTINUE:
			puz = getPreferences(MODE_PRIVATE).getString(PREF_PUZZLE, easyPuzzle); break;
	...
	private int[] calculateUsedTiles(int x, int y){
		...
		//same cell block  (the 3 by 3 block)
		int startx = (x/3) * 3;
		int starty = (y/3) * 3;
		...
		//compress
		int notUsed = 0;
		for(int cellTile:cell){
			if(cellTile!= 0)
				notUsed++;
		}
		
		int c1[] = new int[notUsed];
		notUsed = 0;
		for(int cellTile:cell){
			if(cellTile!=0)
				c1[notUsed++] = cellTile;
